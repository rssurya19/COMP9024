// Binary Search Tree ADT implementation ... COMP9024 24T1
...
Tree partition(Tree t, int m){
    int n = TreeNumNodes(left(t));
    if (m<n) {
        left(t) = partition(left(t), m);
        t = rotateRight(t);
    } else if (m>n){
        right(t) = partition(right(t), m-n-1);
        t= rotateLeft(t);
    }
    return t;
}
